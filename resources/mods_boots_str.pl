...
mod_group(boots_str, 'PercentIncreasedArmourFlattoStunThreshold', '#% increased Armour, # to Stun Threshold', [armour, stun], 1000, 100, 6, prefix).
...
mod_group(boots_str, 'FlatLifeRegenerationPerSecond', '# Life Regeneration per second', [life, regeneration], 1000, 100, 8, suffix).
mod_group(boots_str, 'PercentIncreasedRarityOfItemsFound', '#% increased Rarity of Items found', [rarity], 1000, 100, 3, suffix).
mod_group(boots_str, 'PercentReducedAttributeRequirements', '#% reduced Attribute Requirements', [], 750, 100, 5, suffix).
mod_group(boots_str, 'PercentReducedChillDurationOnYou', '#% reduced Chill Duration on you', [cold], 500, 100, 5, suffix).
mod_group(boots_str, 'PercentReducedFreezeDurationOnYou', '#% reduced Freeze Duration on you', [cold], 500, 100, 5, suffix).
mod_group(boots_str, 'PercentReducedShockDurationOnYou', '#% reduced Shock duration on you', [lightning], 500, 100, 5, suffix).
mod_group(boots_str, 'FlattoStrength', '# to Strength', [attribute], 1000, 100, 8, suffix).
mod_group(boots_str, 'FlattoStunThreshold', '# to Stun Threshold', [stun], 800, 100, 11, suffix).
