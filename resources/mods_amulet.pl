...
mod_pool_weight_sum(amulet, prefix, 9900).
mod_pool_weight_sum(amulet, suffix, 16650).
...
mod_group(amulet, 'PercentIncreasedRarityOfItemsFound', '#% increased Rarity of Items found', [rarity], 1000, 100, 3, prefix).
mod_group(amulet, 'PercentIncreasedSpellDamage', '#% increased Spell Damage', [spell, damage], 1000, 100, 6, prefix).
mod_group(amulet, 'FlattoAccuracyRating', '# to Accuracy Rating', [accuracy], 800, 100, 8, prefix).
...
mod_group(amulet, 'FlattoSpirit', '# to Spirit', [], 500, 100, 5, prefix).

mod_group(amulet, 'FlatLifeRegenerationPerSecond', '# Life Regeneration per second', [life, regeneration], 1000, 100, 10, suffix).
mod_group(amulet, 'PercentIncreasedCastSpeed', '#% increased Cast Speed', [caster, speed], 800, 100, 5, suffix).
mod_group(amulet, 'PercentIncreasedCriticalDamageBonus', '#% increased Critical Damage Bonus', [critical, damage], 1000, 100, 6, suffix).
mod_group(amulet, 'PercentIncreasedCriticalHitChance', '#% increased Critical Hit Chance', [critical], 1000, 100, 6, suffix).
mod_group(amulet, 'PercentIncreasedManaRegenerationRate', '#% increased Mana Regeneration Rate', [mana, regeneration], 1000, 100, 6, suffix).
mod_group(amulet, 'PercentIncreasedRarityOfItemsFound', '#% increased Rarity of Items found', [rarity], 1000, 100, 3, suffix).
...
mod_group(amulet, 'FlattoLevelOfAllMeleeSkills', '# to Level of all Melee Skills', [], 500, 100, 3, suffix).
mod_group(amulet, 'FlattoLevelOfAllMinionSkills', '# to Level of all Minion Skills', [], 500, 100, 3, suffix).
mod_group(amulet, 'FlattoLevelOfAllProjectileSkills', '# to Level of all Projectile Skills', [], 500, 100, 3, suffix).
mod_group(amulet, 'FlattoLevelOfAllSpellSkills', '# to Level of all Spell Skills', [spell], 500, 100, 3, suffix).
