...
mod_group(boots_dex_int, PercentIncreasedEvasionAndEnergyShield, '#% increased Evasion and Energy Shield', [evasion, energy_shield], 1000, 100, 7, prefix).
mod_group(boots_dex_int, PercentIncreasedEvasionAndEnergyShieldFlattoStunThreshold, '#% increased Evasion and Energy Shield, # to Stun Threshold', [evasion, energy_shield], 1000, 100, 6, prefix).
mod_group(boots_dex_int, PercentIncreasedMovementSpeed, '#% increased Movement Speed', [speed], 1000, 100, 6, prefix).
mod_group(boots_dex_int, FlattoEvasionRatingFlattoMaximumEnergyShield, '# to Evasion Rating, # to maximum Energy Shield', [evasion, energy_shield], 1000, 100, 4, prefix).
...
mod_group(boots_dex_int, FlatLifeRegenerationPerSecond, '# Life Regeneration per second', [life], 1000, 100, 8, suffix).
mod_group(boots_dex_int, PercentIncreasedRarityOfItemsFound, '#% increased Rarity of Items found', [], 1000, 100, 3, suffix).
mod_group(boots_dex_int, PercentReducedAttributeRequirements, '#% reduced Attribute Requirements', [attribute], 750, 100, 5, suffix).
mod_group(boots_dex_int, PercentReducedChillDurationOnYou, '#% reduced Chill Duration on you', [], 500, 100, 5, suffix).
mod_group(boots_dex_int, PercentReducedFreezeDurationOnYou, '#% reduced Freeze Duration on you', [], 500, 100, 5, suffix).
mod_group(boots_dex_int, PercentReducedShockDurationOnYou, '#% reduced Shock duration on you', [], 500, 100, 5, suffix).
...
mod_group(boots_dex_int, FlattoStunThreshold, '# to Stun Threshold', [], 800, 100, 11, suffix).
