...
mod_group(boots_dex, PercentIncreasedEvasionRatingFlattoStunThreshold, '#% increased Evasion Rating, # to Stun Threshold', [evasion], 1000, 100, 6, prefix).
...
mod_group(boots_dex, FlatLifeRegenerationPerSecond, '# Life Regeneration per second', [life], 1000, 100, 8, suffix).
mod_group(boots_dex, PercentIncreasedRarityOfItemsFound, '#% increased Rarity of Items found', [], 1000, 100, 3, suffix).
mod_group(boots_dex, PercentReducedAttributeRequirements, '#% reduced Attribute Requirements', [attribute], 750, 100, 5, suffix).
mod_group(boots_dex, PercentReducedChillDurationOnYou, '#% reduced Chill Duration on you', [], 500, 100, 5, suffix).
mod_group(boots_dex, PercentReducedFreezeDurationOnYou, '#% reduced Freeze Duration on you', [], 500, 100, 5, suffix).
mod_group(boots_dex, PercentReducedShockDurationOnYou, '#% reduced Shock duration on you', [], 500, 100, 5, suffix).
mod_group(boots_dex, FlattoDexterity, '# to Dexterity', [attribute], 1000, 100, 8, suffix).
mod_group(boots_dex, FlattoStunThreshold, '# to Stun Threshold', [], 800, 100, 11, suffix).
