...
mod_group(boots_int, 'PercentIncreasedEnergyShieldFlattoStunThreshold', '#% increased Energy Shield, # to Stun Threshold', [energy_shield, stun], 1000, 100, 6, prefix).
...
mod_group(boots_int, 'FlatLifeRegenerationPerSecond', '# Life Regeneration per second', [life, regeneration], 1000, 100, 8, suffix).
mod_group(boots_int, 'PercentIncreasedRarityOfItemsFound', '#% increased Rarity of Items found', [rarity], 1000, 100, 3, suffix).
mod_group(boots_int, 'PercentReducedAttributeRequirements', '#% reduced Attribute Requirements', [], 750, 100, 5, suffix).
mod_group(boots_int, 'PercentReducedChillDurationOnYou', '#% reduced Chill Duration on you', [cold], 500, 100, 5, suffix).
mod_group(boots_int, 'PercentReducedFreezeDurationOnYou', '#% reduced Freeze Duration on you', [cold], 500, 100, 5, suffix).
mod_group(boots_int, 'PercentReducedShockDurationOnYou', '#% reduced Shock duration on you', [lightning], 500, 100, 5, suffix).
mod_group(boots_int, 'FlattoIntelligence', '# to Intelligence', [attribute], 1000, 100, 8, suffix).
mod_group(boots_int, 'FlattoStunThreshold', '# to Stun Threshold', [stun], 800, 100, 11, suffix).
